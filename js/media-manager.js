...
( function( $ ) {
	...
	ds.media = media = {
		...
		detailsContainerId: '#map-container',
		...
		handleMediaAttachment: function( attachment ) {
			...
			var full = attachment.get( 'sizes' ).full;
			$( 'img', details ).attr( 'src', full.url ).css( 'display', 'none' );
			$( '#sample-height' ).attr( 'value', full.height);
			$( '#sample-width' ).attr( 'value', full.width);
			$( '#sample-ratio').attr( 'value', full.width / full.height);
		}
	};
